...
- [ ] **Phase README** (`AGENTS/Phase X/<Phase Name>/README.md`) updated with:
...
- [ ] **Phase CHECKLIST** (`AGENTS/Phase X/<Phase Name>/CHECKLIST.md`) has:
...
- `.github/agents/<agent-name.agent.md>` - <What this agent does based on its awesome-copilot description>
  - Task 1: <Specific task from agent description>
  - Task 2: <Specific task from agent description>
  - Task 3: <Specific task from agent description>
```

**IMPORTANT:** Replace ALL `<placeholder>` text with actual content from the agent's description in awesome-copilot.
...
### Actions
- [ ] <Specific actionable task 1 for this agent - based on what this agent does in awesome-copilot>
- [ ] <Specific actionable task 2 for this agent>
- [ ] <Specific actionable task 3 for this agent>

### Outputs
- [ ] Deliverable: <path to expected output file/folder>

### Exit Checks
- [ ] <Validation condition 1 - e.g., Build passes>
- [ ] <Validation condition 2 - e.g., Tests pass>
...
### Evidence
- Artifacts: <paths of files created/modified>
...
**IMPORTANT:** Replace ALL `<placeholder>` text with actual content:
- `<agent-name.agent.md>` → actual agent filename (e.g., `expert-react-developer.agent.md`)
- `<Specific actionable task 1>` → actual task from the agent's description in awesome-copilot
- `<path to expected output>` → actual file path the agent would create
- `<Validation condition>` → actual validation (e.g., "npm run build passes")
...
1. **Current Phase** - Update Phase 3:
   - Status: `completed`
   - Phase Started At: `<actual date>`
   - Target Exit Date: `<actual date or remove>`
...
2. **Current Agent Ownership** - Mark as complete:
   ...
   - Ownership Last Changed At: `<timestamp>`
...
5. **Last Completed Item** - Add final entry:
   - `Phase 3 complete: <N> agents, <M> hooks, <K> workflows imported and phase docs updated`
...
- [ ] **Phase CHECKLIST** (`AGENTS/Phase X/<Phase Name>/CHECKLIST.md`) updated with:
